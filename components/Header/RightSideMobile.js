    ...
    <div className='inline-flex sm:hidden items-center justify-center '>
      ...
      >
        ...
        <svg
          ...
          className={`${mobileMenuIsActive ? 'block' : 'hidden'} h-6 w-6`}
          stroke='currentColor'
          fill='none'
          viewBox='0 0 24 24'
        >
  ...
  return (
    <div ref={mobileMenuNode} className={`${mobileMenuIsActive ? 'block' : 'hidden'} sm:hidden`}>
      ...
      <div className='pt-3 pb-3 border-t border-gray-200'>
        {userData.isLoading ? (
          <Skeleton className='ml-4' width={150} height={24} />
        ) : userData.isAuth ? (
          // logged in
          <>
            <div className='flex items-center px-4 text-sm font-medium leading-5 text-gray-500'>
              {userData.data.email}
            </div>
